...
<el-dialog title="取消任务" :visible.sync="dialogFormVisible1">
  ...
  <div slot="footer" class="dialog-footer">
    ...
    <el-button type="primary" @click="handleCancel(scope.$index, scope.row)">确 定</el-button>
  </div>
</el-dialog>
      <br /><br />
...
<el-dialog title="任务反馈" :visible.sync="dialogFormVisible2">
  ...
  <div slot="footer" class="dialog-footer">
    ...
    <el-button type="primary" @click="handleBack(scope.$index, scope.row)">确 定</el-button>
  </div>
</el-dialog>
      <br /><br />
...
export default {
  ...
  data() {
    ...
    this.detail=this.$route.query.detail;   
      ...
      var self=this;
        axios.post('http://localhost:8081/qujin/client/task/receiver/cancle',sendJson)
           .then(response => {
    ...
    handleBack(index,row){
     var sendJson = this.upform2;
      var self=this;
        axios.post('http://localhost:8081/qujin/client/feedback/add',sendJson)
           .then(response => {
              self.result=JSON.parse(JSON.stringify(response.data));
              if(self.result=="success")
              {
    ...
    compelete(){
    this.tid.id=this.detail.id;
    var sendJson = this.tid;
      var self=this;
        axios.post('http://localhost:8081/qujin/client/task/receiver/accomplish',sendJson)
           .then(response => {
              self.result=JSON.parse(JSON.stringify(response.data));
              if(self.result=="success")
              {
